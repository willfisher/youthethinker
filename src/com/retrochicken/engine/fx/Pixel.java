...
public class Pixel {
	...
	public static final int TRANSPARENT = getColor(0, 1, 1, 1);
	...
	public static int getLightBlend(int color, int light, int ambientLight, ShadowType shadowType) {
		if(shadowType == ShadowType.UNAFFECTED)
			return color;
	...
	public static int getColorBlend(int color1, int color2) {
		return getColor(1, getRed(color1) * getRed(color2), getGreen(color1) * getGreen(color2), getBlue(color1) * getBlue(color2));
	}
